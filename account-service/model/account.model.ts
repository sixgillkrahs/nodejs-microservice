...
class AccountModel {
    ...
    constructor(logger: LoggerInstance, dbConnection: Connection | null, plugins: any[] = []) {
        ...
        this.model = this.dbConnection?.model<Entities.AccountEntity>(DB_COLLECTION.ACCOUNT, this.schema);
    }
    ...
    public async listPaging(filter: object, sort: object, pageIndex: number = 0, limit: number = 20, select: object = {}) {
        if (_.isEmpty(sort)) {
            sort = { createdAt: -1 };
        }
        if (!filter) {
    ...
    public async findOne(filter: object | any, isWithOutCheckDelete: boolean = false, select: object = {}) {
